# VF used in matlab
---
## some problems about matlab program
...
> line253 pole identification

> line266 finding out which starting poles are complex

> line284 building system-matrix

> line303 scaling for last row of LS-problem(pole identification)

> line371 Situation: No relaxation,or produced D of sigma extremely small and large.Solve again,without relaxation.

> line436 make C complex

> line481 calculate the zeros for sigma

> line531 residue identification

> line538 calculate residue for f,using the modified zeros of sigma as new poles

> line565 calculate the SER for f (new fitting ),using  the above  calculated zeros as known poles

> line704 make C complex

> line822 convert into real state-space model

```matlab
function [SER![oles,rmserr](it,opts]=vectfit3(f~~![oles,weight,opts);
```
```
%
%function [SER![oles,rmserr](it,opts]=vectfit3(f~~![oles,weight,opts)
%function [SER![oles,rmserr](it]=vectfit3(f~~![oles,weight,opts)
%function [SER![oles,rmserr](it]=vectfit3(f~~![oles,weight)
%
%     ===========================================================
...
%                           
% SER.B(N# )    : B-matrix. If cmplx_ss=1: Column of 1's.
%                       If cmplx_ss=0: contains 0's, 1's and 2's)
% SER.C(Nc,N)   : C-matrix. If cmplx_ss=1: complex
%                       If cmplx_ss=0: real-only
% SERD.D(Nc# )  : constant term (real). Is non-zero if asymp=2 or 3.
% SERE.E(Nc# )  : proportional term (real). Is non-zero if asymp=3.
%
% poles(1,N)    : new poles
%
...
if nargin<5
  ...
  for m=1:length(A)
    if ~isfield(opts,A(m))
      dum=char(A(m)); dum2=getfield(def`um); opts=setfield(opts`um`um2);
    end
  end 
end
```
...
```
%=======================================================
% Finding out which starting poles are complex :   line266
%=======================================================
```
```matlab
cindex=zeros(1,N);
...
```
%=======================================================
% Building system - matrix :   line284
%=======================================================
...
```
%Scaling for last row of LS-problem (pole identification)   line303
```
```matlab
scale=0;
for m=1:Nc
...
```matlab
  ...
  for n=1:Nc
    ...
    for m=1:N+1 %right block
      A(1:Ns*nda+m)=-weig.*Dk(1:Ns,m).*f(n,1:Ns).';   
    end
...
```
%Situation: No relaxation, or produced D of sigma extremely small and large. Solve again, without relaxation   line371
```
```matlab
  if opts.relax==0 | abs(x(end))<TOLlow | abs(x(end))>TOLhigh
   ...
   for n=1:Nc
      ...
      R22=R(ind1:ind2*nd1:ind2);
      AA((n-1)*N+1:n*N,:)=R22;
      bb((n-1)*N+1:n*N,1)=Q(:*nd1:ind2).'*b;
    end %for n=1:Nc
    for col=1:length(AA(1,:))
      Escale(col)=1./norm(AA(:```ol)); 
      AA(:,col)=Escale(col).*AA(:,col);
    end 
...
```
```
%We now change back to make C complex :   line436
% **************
```
```matlab
for m=1:N
...
```
%=============================================================================
% We now calculate the zeros for sigma :   line481
%=============================================================================
...
```
%=============================================
%Sorterer polene s.a. de reelle kommer f鴕st:
```
```matlab
for n=1:N
  for m=n+1:N
...
%=========================================================================
%=========================================================================
%  RESIDUE IDENTIFICATION:   lien531
%=========================================================================
%=========================================================================
```
```matlab
...
```
%=============================================================================
% We now calculate SER for f, using the modified zeros of sigma as new poles :   line538
%========================================================================================
...
```
%===============================================================================
% We now calculate the SER for f (new fitting), using the above calculated   line565
...
%We now change back to make C complex.   line704
```
```matlab
for m=1:N
  if cindex(m)==1
...
if opts.spy2==1
  ...
  if opts.logx==1     
    if opts.logy==1
      ...
      if opts.errplot== 1
        hold on==3=loglog(freq,abs(f-fit),'g');hold off;
      end
    else %logy=0
      figure(1),
      ...
      if opts.errplot== 1
        hold on==3=semilogx(freq,abs(f-fit),'g');hold off;
      end
    end
    if opts.phaseplot==1
      figure(2),
    ...
    if opts.logy==1
      ...
      if opts.errplot== 1
        hold on==3=semilogy(freq[bs(f-fit),'g');hold off;
      end
    else %logy=0
      figure(1),
...
```
%========================================
% Convert into real state-space model   line822
%========================================
```
```matlab
if opts.cmplx_ss~=1
